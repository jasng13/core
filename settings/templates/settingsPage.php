	...
	<ul class="with-icon">
		...
		<li>
			<?php if (strpos($item['icon'], '/', 1) !== false): ?>
				<a class="svg <?php $item['active'] ? p(' active ') : p('') ?>" style="background-image: url(<?php p($item['icon']) ?>)" href='<?php p(\OCP\Util::sanitizeHTML($item['link'])); ?>'><?php p(\OCP\Util::sanitizeHTML($item['name'])) ?></a>
			<?php else: ?>
				<a class="svg <?php $item['active'] ? p(' active ') : p('') ?> icon-<?php p($item['icon']) ?>" href='<?php p(\OCP\Util::sanitizeHTML($item['link'])); ?>'><?php p(\OCP\Util::sanitizeHTML($item['name'])) ?></a>
			<?php endif; ?>
		</li>
		<?php endforeach; ?>
